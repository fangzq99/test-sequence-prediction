...
context('Worksite tests (Create)', () => {
    worksiteCreateUsers.forEach(user => {
        context(user.role, () => {
            ...
            it('Successfully create a demo worksite without lift support, then delete the same worksite afterwords as users with delete worksite rights, then verify the delete worksite api call', () => {
                worksiteCreate.createDemoWorksiteWithoutLiftSupport(user.role)
            })
