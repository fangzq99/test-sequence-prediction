...
context('Deployments tests (R3)', () => {
    deploymentUsers.forEach(user => {
        context(user.role, () => {
            ...
            it('Search and select R3, then search and select worksite, in assign users page, search each role, select all users in that role then deselect them', () => {
                deploymentR3.assignUsersToRobot(r3DeploymentSearch)
            })
        })
    })
})
